...
class Organizator(models.Model):
    ...
    jmbg = models.BigIntegerField(db_column='JMBG', unique=True)  # Field name made lowercase.
...
class PrijaveOrganizatora(models.Model):
    ...
    jmbg = models.IntegerField(db_column='JMBG', unique=True)  # Field name made lowercase.
...
class PrijaveZaOglas(models.Model):
    ...
    idoglas = models.OneToOneField(Oglas, models.DO_NOTHING, db_column='IdOglas', primary_key=True)  # Field name made lowercase. The composite primary key (IdOglas,IdKor) found, that is not supported. The first column is selected.

    class Meta:
        managed = False
        db_table = 'prijave_za_oglas'
        unique_together = (('idoglas', 'idkor'),)
